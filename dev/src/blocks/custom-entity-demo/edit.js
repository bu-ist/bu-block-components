            ...
            <div className="custom-entity-demo">
                ...
                {regularPostsLoading ? (
                    ...
                    <div className="post-list">
                        <h4>{__('Posts from standard WP entity', 'imports-dev')}</h4>
                ...
                {customPostsLoading ? (
                    ...
                    <div className="post-list">
                        <h4>{__('Posts from custom entity', 'imports-dev')}</h4>
                        {customPosts && customPosts.length ? (
                            ...
                            <p>{__('No posts found from custom entity.', 'imports-dev')}</p>
                        )}
                        <button
                ...
                <h3>{__('Method 3: Custom Entity with Meta Value Query', 'imports-dev')}</h3>
                {smithPostsLoading ? (
                    <LoadingSpinner text="Loading filtered posts..." />
                ) : (
                    <div className="post-list">
                        <h4>{__('Posts with last name "Smith"', 'imports-dev')}</h4>
                        {smithPosts && smithPosts.length ? (
                            ...
                            <p>{__('No posts found with last name "Smith". You need to add some posts with _bob_last_name = Smith meta.', 'imports-dev')}</p>
                        )}
                    </div>
                )}

                <h3>{__('Method 4: Custom Entity with Meta Exists Check', 'imports-dev')}</h3>
                {postsWithLastNameLoading ? (
                    <LoadingSpinner text="Loading posts with meta key..." />
                ) : (
                    <div className="post-list">
                        <h4>{__('Posts with any last name value', 'imports-dev')}</h4>
                        {postsWithLastName && postsWithLastName.length ? (
                            <ul>
                                {postsWithLastName.map(post => {
                                    // Get the last name value from meta if available
                                    const lastName = post.meta &&
                                        post.meta._bob_last_name &&
                                        post.meta._bob_last_name[0] ?
                                        post.meta._bob_last_name[0] : 'unknown';

                                    return (
                                        <li key={post.id}>
                                            <strong>{post.title.rendered}</strong>
                                            <em> (has _bob_last_name = {lastName})</em>
                                        </li>
                                    );
                                })}
                            </ul>
                        ) : (
                            <p>{__('No posts found with any last name value. You need to add some posts with _bob_last_name meta.', 'imports-dev')}</p>
                        )}
                    </div>
                )}
                ...
                {postId && (
                    <div className="selected-post">
                        <h3>{__('Selected Post Details', 'imports-dev')}</h3>
                        {selectedPostLoading ? (
