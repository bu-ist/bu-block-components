 ...
 * @return {Object} Option data to use with SelectControl.
 */
	...
	return {
		label:
			...
			decodeEntities(
				title.rendered ||
					sprintf(
						__( '#%d (no title)', 'block-editor-components' ),
						id
					)
			),
		value: id,
	...
	return {
		label:
			...
			decodeEntities(
				name ||
					sprintf(
						__( '#%d (no name)', 'block-editor-components' ),
						id
					)
			),
		value: id,
 ...
 * @return {object[]} Options data to use with SelectControl.
 */
 ...
 * @return {object[]} Options data to use with SelectControl.
 */
export function createOptionsFromPostsWithHierarchy(
	posts,
	prefix = '— ',
	level = 0
) {
 ...
 * @return {object[]} Options data to use with SelectControl.
 */
export function createOptionsFromTermsWithHierarchy(
	terms,
	prefix = '— ',
	level = 0
) {
